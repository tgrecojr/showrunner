...
describe('api client', () => {
  ...
  it('throws Error("API <status>: <error>") when JSON body has error field', async () => {
    fetchSpy.mockResolvedValueOnce(jsonResponse(404, { error: 'show 9 missing' }));
    await expect(api.getShow(9)).rejects.toThrowError('API 404: show 9 missing');
  });
  ...
  it('sync hits POST /sync', async () => {
    fetchSpy.mockResolvedValueOnce(jsonResponse(200, { shows_synced: 0, errors: [] }));
    await api.sync();
    const [url, init] = fetchSpy.mock.calls[0] as FetchArgs;
